...
def merge_percentage_cells(table):
    """
    Merge % cells vertically in column 17 (% column).

    Similar to campaign merging, this function merges percentage cells from the start
    of a campaign until the cell before the MONTHLY TOTAL gray row. This creates a
    single merged cell for the campaign budget percentage.
    ...
    merge_start = None
    merges_performed = 0
    campaign_rows_found = 0
    monthly_total_rows_found = 0
    ...
    for row_idx in range(1, row_count):
        ...
        if actual_campaign_name and not is_monthly and not is_grand and not is_gray:
            campaign_rows_found += 1
            if merge_start is None:
                merge_start = row_idx
                logger.debug(f"Found % merge start at row {row_idx}: {actual_campaign_name}")

        # Perform merge when we hit a GRAY MONTHLY TOTAL row
        if is_monthly and is_gray:
            monthly_total_rows_found += 1
            if merge_start is not None:
                merge_end = row_idx - 1  # Merge until row before MONTHLY TOTAL

                logger.debug(f"Hit gray MONTHLY TOTAL at row {row_idx}, attempting merge from {merge_start} to {merge_end}")
                ...
                if merge_end > merge_start:
                    try:
                        ...
                        if not _cells_are_same(top_cell, bottom_cell):
                            top_cell.merge(bottom_cell)
                            merges_performed += 1
                            logger.debug(f"Merged % cells rows {merge_start}-{merge_end}")
                        ...
                        logger.error(f"Failed to merge % cells rows {merge_start}-{merge_end}: {e}")
                else:
                    logger.debug(f"Skipping merge: merge_end ({merge_end}) not > merge_start ({merge_start})")

                merge_start = None

    logger.info(f"Percentage merges completed: {merges_performed} merge(s) (campaigns: {campaign_rows_found}, monthlies: {monthly_total_rows_found})")
    return merges_performed
