...
- Regenerated deck `GeneratedDeck_Task11_fixed.pptx` under `output/presentations/run_20251020_124516/` (114 slides); COM automation + python-pptx confirm slide count.
...
- Added `features.clone_pipeline_enabled` flag to `master_config.json` so operators can disable the clone workflow and fall back to AutoPPTX if needed.
...
- Extend regression suite (tiles formatting, duplication guards).
