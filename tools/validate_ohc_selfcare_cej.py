#!/usr/bin/env python3
"""
Validate CEJ calculations for OHC (Oral Health Care) and Self Care brands.

OHC = Oral Health:
- Sensodyne, Parodontax, Aquafresh, Pronamel, Corega, Polident

Self Care = Pain Management + Respiratory:
- Pain: Panadol, Voltaren, Calpol, Grand-Pa
- Respiratory: Otrivin, Theraflu, Med-Lemon, Nicotinell
"""
...
# OHC brands (Oral Health Care)
OHC_BRANDS = ["SENSODYNE", "PARODONTAX", "AQUAFRESH", "PRONAMEL", "COREGA", "POLIDENT"]

# Self Care brands (Pain Management + Respiratory)
SELF_CARE_BRANDS = [
    # Pain Management
    "PANADOL", "VOLTAREN", "CALPOL", "GRAND-PA",
    # Respiratory
    "OTRIVIN", "THERAFLU", "MED-LEMON", "NICOTINELL"
]
...
def compute_expected_cej(df: pd.DataFrame, market: str, brand: str, product: str = None) -> dict:
    ...
    if product and product.upper() != "PRODUCT SUMMARY":
        ...
        # Handle renamed products (e.g., "Sensodyne Product" -> "Sensodyne")
        rename_map = {
...
def validate_category(prs, df, brand_list, category_name):
    """Validate CEJ for a category of brands."""
    print("=" * 80)
    print(f"{category_name} CEJ VALIDATION")
    print("=" * 80)
    ...
    for slide_num, slide in enumerate(prs.slides, start=1):
        ...
        # Check if this brand matches category
        is_match = any(b in brand_upper for b in brand_list)

        if not is_match:
            continue

        category_slides.append(slide_num)
        ...
        if not actual:
            ...
            print(f"  (No CEJ values - likely page 1 of multi-slide)")
            print()
            continue
    ...
    print(f"  Slides found: {len(category_slides)}")
    print(f"  Slides with CEJ: {total}")
    print(f"  Passed: {passed}")
    print(f"  Failed: {failed}")

    if failed == 0:
        print(f"  ✅ ALL {category_name} CEJ CORRECT!")
    else:
        print(f"  ❌ SOME {category_name} CEJ MISMATCHES")
        for r in results:
            if not r["match"]:
...
def main():
    ...
    # Validate OHC
    ohc_results = validate_category(prs, df, OHC_BRANDS, "OHC (ORAL HEALTH)")

    # Validate Self Care
    selfcare_results = validate_category(prs, df, SELF_CARE_BRANDS, "SELF CARE")

    # Grand total
    print("=" * 80)
    print("GRAND TOTAL")
    print("=" * 80)
    all_results = ohc_results + selfcare_results
    total = len(all_results)
    passed = sum(1 for r in all_results if r["match"])
    failed = total - passed

    print(f"Total slides validated: {total}")
    print(f"Passed: {passed}")
    print(f"Failed: {failed}")
    print()

    if failed == 0:
        print("✅ ALL OHC + SELF CARE CEJ CALCULATIONS ARE CORRECT!")
    else:
        print("❌ SOME CEJ CALCULATIONS HAVE MISMATCHES")
